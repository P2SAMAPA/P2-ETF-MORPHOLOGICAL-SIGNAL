...
import config
import data_manager
from morphology import compute_morphological_score
...
def main():
    ...
    for universe_name, tickers in config.UNIVERSES.items():
        print(f"\n=== Universe: {universe_name} (Morphological Signal) ===")
        ...
        for win in config.WINDOWS:
            ...
            for etf in tickers:
                if etf not in prices_win.columns:
                    continue
                series = prices_win[etf].dropna().values
                if len(series) < config.STRUCTURING_SIZE + 2:
                    continue
                score = compute_morphological_score(
                    series,
                    se_size=config.STRUCTURING_SIZE,
                    se_type=config.STRUCTURING_ELEMENT,
        ...
        top_etfs = [{"ticker": ticker, "morph_score": float(score), "best_window": win} for ticker, (score, win) in sorted_etfs[:config.TOP_N]]

        print(f"  Top 3 ETFs by morphological score: {[e['ticker'] for e in top_etfs]}")
